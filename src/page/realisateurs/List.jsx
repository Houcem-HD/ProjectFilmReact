...
const DatatableTables = () => {
  ...
  const handleDelete = async (realisateursID) => {
    try {
      // Perform delete request
      const response = await axiosIns.delete(`realisateurs/${realisateursID}`);

      if (response.status === 200) {
        // Remove the deleted item from the state
        setData((prevData) => prevData.filter((item) => item.realisateursID !== realisateursID));
      }
    } catch (error) {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div>
            {/* Edit Button */}
            <Link
              to={`/realisateursEdit/${row.original.realisateursID}`}
              className="btn btn-primary mr-2 bx bx-edit"
            ></Link>

            {/* Delete Button */}
            <button
              onClick={() => handleDelete(row.original.realisateursID)}
